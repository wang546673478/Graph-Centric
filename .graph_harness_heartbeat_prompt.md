...
### 第1步：建图（必须产生节点和边）
用这些节点类型建图：
- `Component` — Vue SFC 组件 (如 `Transcript.vue`)
- `Style` — CSS 变量或样式块 (如 `--bg: #0f172a`)
- `Layout` — 布局结构 (如 `flex` 容器)
- `Page` — 路由页面 (如 `/settings`)
- `Task` — 工作单元

**建图示例（第1轮 - 配色系统）：**
```
A (anchor, immutable): Component "RunView.vue" — 当前使用硬编码颜色
D (goal): Style "optimized-colors" — 统一的暗色 CSS 变量体系
边: D -[DependsOn]-> A

中间节点:
B1: Style "dark-theme-tokens" — --bg, --text, --accent 等变量定义
B2: Component "main.css" — 全局样式入口
边: B1 -[DependsOn]-> A, D -[DependsOn]-> B1
```

### 第2步：执行
1. Explore (可选): 搜索外部项目获取设计灵感
2. ProposePatch: 仅修改 webui/src/ 下文件
3. SubAgent 执行修改(edit_file/write_file)
4. Review 通过 → 本轮完成

### 建图检查清单
- [ ] A 节点标记了 `immutable: true`？
- [ ] D 节点描述了目标效果？
- [ ] 有 `DependsOn` 边连接 D→...→A？
- [ ] 中间节点使用了正确的类型(Component/Style/Layout)？

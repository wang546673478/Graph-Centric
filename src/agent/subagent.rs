...
const SUBAGENT_SYSTEM_PROMPT_DEFAULT: &str = "# max_steps=8 / emit final_answer 警告
    ...
    let directive = match task_kind {
        ...
        TaskKind::Modify => String::from("\n\n🔧 **THIS IS A MODIFY TASK. CRITICAL: you DO NOT have `read_file`/`edit_file`/`write_file` directly —\n\
                             ...
                             1) `explore` (1 item): find the exact file path + line range to change. e.g.\n\
                                `{\"items\":[{\"scope\":\"src/agent/foo.rs\", \"question\":\"Locate the `bar` function and return the exact line range (start-end, 1-based) plus the existing signature\"}]}`\n\
                             2) `propose_patch`: add a complex node named after the file (e.g. `modify-foo-bar`) with:\n\
                                - `kind: \"Task\"` (NOT \"File\" — the file is what we're creating, not the task)\n\
                                - `summary: \"修改 <file>: <specific change>\"\n\
                                - a `drill_down: { target: \"modify-foo-bar\", reason: \"<one sentence>\", sub_task_override: \"<the modify instruction>\" }`\n\
                             3) `ready_for_verify` once the drill-down node is on the graph.\n\

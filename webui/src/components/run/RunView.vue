...
function handleEvent(e: WSEvent) {
  switch (e.type) {
    case 'transcript': transcript.value.push({ role: e.role || 'assistant', content: e.content || '' }); break
    case 'graph': case 'graph_snapshot': if (e.nodes) nodes.value = e.nodes; if (e.edges) edges.value = e.edges; break
    case 'status': if (e.phase) status.value = e.phase; if (e.message) statusMsg.value = e.message; if (e.tokens_used) tokensUsed.value = e.tokens_used; break
    case 'done': status.value = 'Done'; break
    case 'error': errorMsg.value = e.message || 'Unknown error'; status.value = 'Error'; break
    case 'cascade_step': if (detailMode.value) transcript.value.push({ role: 'cascade', content: `🔍 ${e.changed_node} ← ${e.predecessor}: ${e.verdict} — ${e.rationale}` }); break
    case 'model_call': if (detailMode.value) transcript.value.push({ role: 'model', content: `🤖 ${e.component} (${e.completion_tokens || 0}t, ${e.duration_ms || 0}ms): ${(e.response_content || '').slice(0, 200)}` }); break
    case 'checkpoint': transcript.value.push({ role: 'checkpoint', content: `📸 #${e.index} · r${e.round} · ${e.node_count}n/${e.edge_count}e` }); break
  }
}
...
async function submitTask(task: string) {
  ...
  try {
    ...
    activeRunId.value = id; status.value = 'Running'
    transcript.value = [{ role: 'user', content: task }]
    nodes.value = []; edges.value = []; tokensUsed.value = 0
    if (socket) socket.disconnect()
    socket = useRunSocket(id, handleEvent)
  } catch (e: any) { errorMsg.value = String(e) } finally { sending.value = false }
}
</script>
